...
const int delayMS = 50; // variable for delay time (in milliseconds)
...
void loop() 
{
  // for loop fades LED value from 0 to 255, incrementing by 1
  for (int i = 0 ; i <= 255; i += 5) 
  { 
    analogWrite(LED, i); //writes value of i to pin
    delay(delayMS);  // short delay so LED effect is visible
  }

  for (int i = 255 ; i >= 0; i -= 5) 
  { // for loop fades LED value from 255 to 0, de-incrementing by 1
    analogWrite(LED, i);
    delay(delayMS);
  }
}
